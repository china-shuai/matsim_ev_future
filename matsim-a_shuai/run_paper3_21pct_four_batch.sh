...
MVN_BIN="${MVN_BIN:-$(command -v mvn)}"
CAFFEINATE_BIN="${CAFFEINATE_BIN:-$(command -v caffeinate)}"
...
OUTPUT_ROOT="/Users/S4065267/Simulation_paper3/output/batch_${RUN_ID}_21pct_four"
LOG_DIR="${OUTPUT_ROOT}/logs"
mkdir -p "$LOG_DIR"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] BUILD current EV and Paper 3 modules"
"$MVN_BIN" -q -pl contribs/ev,matsim-a_shuai -am -DskipTests install
...
run_case() {
	...
	"$CAFFEINATE_BIN" -dimsu "$MVN_BIN" -q -f matsim-a_shuai/pom.xml -DskipTests exec:java \
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] 21PCT PAPER 3 RANDOM FOUR-SCENARIO RUNS DONE"

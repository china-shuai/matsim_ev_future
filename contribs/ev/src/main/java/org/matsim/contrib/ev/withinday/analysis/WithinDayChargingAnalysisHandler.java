...
public class WithinDayChargingAnalysisHandler implements //
	...
	public void handleEvent(StartChargingProcessEvent event) {
		ChargingProcessTracker previous = active.put(event.getVehicleId(), new ChargingProcessTracker(event));

		if (previous != null) {
			LOG.warn("Vehicle {} already had an active charging process. Previous process will be closed as aborted.",
					event.getVehicleId());
			previous.abort = new AbortChargingProcessEvent(event.getTime(), event.getPersonId(),
					event.getVehicleId());
			registerProcess(previous);
		}
	}

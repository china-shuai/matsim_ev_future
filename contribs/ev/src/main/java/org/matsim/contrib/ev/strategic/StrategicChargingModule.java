...
public class StrategicChargingModule extends AbstractModule {
	...
	public void install() {
		...
		if (innovation instanceof RandomChargingPlanInnovator.Parameters || innovation == null) {
			bind(ChargingPlanInnovator.class).to(RandomChargingPlanInnovator.class);
		} else if (innovation instanceof EmptyChargingPlanInnovator.Parameters) {
	...
	RandomChargingPlanInnovator provideRandomChargingPlanCreator(ChargerProvider chargerProvider,
		...
		ChargingInnovationParameters parameters = config.getInnovationParameters();
		if (!(parameters instanceof RandomChargingPlanInnovator.Parameters randomParameters)) {
			throw new IllegalStateException(
					"RandomChargingPlanInnovator requested but innovation parameters are " + parameters.getClass());
		}
		...
		return new RandomChargingPlanInnovator(chargerProvider, candidateFinder, timeInterpretation, config,
				randomParameters, selectorFactory,
